    ...
    */
    public function doalipay(){
        ...
        //这里我们通过TP的C函数把配置项参数读出，赋给$alipay_config；
            ...
            $out_trade_no = 'entefox'.session('user.user_id').time();//商户订单号 通过支付页面的表单进行传递，注意要唯一！
            $subject = '企狐';  //订单名称 //必填 通过支付页面的表单进行传递
    ...
    function notifyurl(){
        ...
        */
        ...
        if($verify_result) {
            ...
            $notify_time    =strtotime($_POST['notify_time']); //时间
            ...
            if($_POST['trade_status'] == 'TRADE_FINISHED') {
                    ...
                    $data['money']=$money+$total_fee;
                    //更新用户信息
                    $this->user->save('user_id='.$user_id,$data);
            }
    ...
    function returnurl(){
        ...
        if($verify_result) {
            ...
            $notify_time    =strtotime($_GET['notify_time']); //时间
            $user_id        =session('user.user_id');
            $username       =session('user.username');
            $parameter = array(
                "out_trade_no"      => $out_trade_no, //商户订单编号；
